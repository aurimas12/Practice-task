...
# Create route
curl -i -X POST http://localhost:8001/services/mockbin/routes \
--data "name=mockbin" \
--data "paths[]=/mockbin"

# Enable the OAuth 2.0 plugin for that service
curl -i -X POST localhost:8001/services/mockbin/plugins \
--data "name=oauth2" \
